...
def prep_data(args):
    ...
    TCGA_PRAD = mapping(args.data_root + "TCGA-PRAD_exprs.z.tsv", mapping_table)
    TCGA_PRAD = TCGA_PRAD.dropna()
    TCGA_KIRC = mapping(args.data_root + "TCGA-KIRC_exprs.z.tsv", mapping_table)
    TCGA_KIRC = TCGA_KIRC.dropna()
    ...
    idx_PRAD = TCGA_PRAD.index
    idx = idx.intersection(idx_PRAD)
    idx_KIRC = TCGA_KIRC.index
    idx = idx.intersection(idx_KIRC)    
    ...
    if drug == "Erlotinib":
        ...
        TCGA_PRAD = pd.DataFrame.transpose(TCGA_PRAD.loc[idx,:])        
        TCGA_KIRC = pd.DataFrame.transpose(TCGA_KIRC.loc[idx,:])        
        ...
        return X_tr, Y_tr, X_U, TCGA_PRAD, TCGA_KIRC, gCSI_exprs_drug, gCSI_aac_drug
